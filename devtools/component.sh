...
start_component(){ [[ -f "$jar" ]]||{ echo "$jar fehlt; zuerst ./mvnw package ausführen." >&2;return 1;};mkdir -p -- "$runtime_root";local pid;pid=$(read_pid "$component" 2>/dev/null||true);if [[ -n "$pid" ]]&&managed_pid "$component" "$pid";then echo "$component läuft bereits (PID $pid).";return 0;fi;rm -f -- "$pid_path";nohup java -jar "$jar" >>"$log_path" 2>&1 & pid=$!;printf '%s\n' "$pid" >"$pid_path.$pid";mv -f -- "$pid_path.$pid" "$pid_path";for ((i=0;i<80;i++));do managed_pid "$component" "$pid"||{ tail -n 8 "$log_path" >&2;rm -f -- "$pid_path";return 1;};local ready=1;for port in $(component_port "$component");do port_listening "$port"||ready=0;done;((ready))&&{ echo "$component gestartet (PID $pid).";return 0;};sleep .1;done;echo "$component Ports nicht bereit; Log: $log_path" >&2;return 1;}
