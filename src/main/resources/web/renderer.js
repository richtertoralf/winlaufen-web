...
function receive(message) {
  ...
  state = message.state;
  if (message.type === 'classSnapshot') liveClassIndex = message.classIndex;
  if (liveClassIndex === null && state.currentFinish) liveClassIndex = state.currentFinish.classIndex;
  renderChrome();
  if (message.type === 'snapshot' || message.type === 'classSnapshot' || !hadState) renderTables();
}
...
Promise.all([fetch('/api/v1/state').then(response => response.json()), fetch('/api/v1/config').then(response => response.json())])
  .then(([value, config]) => { state = value; display = config; render(); connect(config.webSocketPort); });
